...
class MyLinkedList:
    ...
    def addAtHead(self, val: int) -> None:
        ...
        new_node.next = self.linked_list.next # self.linked_list.next (which is the real linked list) is None at the beginning

        if self.linked_list.next:
            self.linked_list.next.prev = new_node
    ...
    def addAtIndex(self, index: int, val: int) -> None:
        ...
        curr = self.linked_list# -1 -> 1 -> 3 -> None (1 is in 0th index position. 3 is in 1st index pòsition)
        i = 0
        while curr:
