class Solution:
    def calPoints(self, operations: List[str]) -> int:
        ...
        for operation in operations:# O(n)
            if operation not in ["C", "D", "+"]:# O(1): Push (append). Since the operation is C, +, D or a number, when operation is a number, then it´s pushed to stack.
